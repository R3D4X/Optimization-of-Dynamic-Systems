...
def test_wolfe(c_1, c_2, banana_a, banana_b, theta, a):
    ...
    p_nd = -1 * fkt.grad(banana_a, banana_b, theta[0], theta[1]);
    new_theta = [0,0];          #Thetabuffer
    new_theta[0] = theta[0] + a * p_nd[0];
    new_theta[1] = theta[1] + a * p_nd[1];
    print("Test Wolfe: Theta = [ " + str(theta[0]) + " , " + str(theta[1]) + "]");
    print("Test Wolfe: New Theta = [ " + str(new_theta[0]) + " , " + str(new_theta[1]) + "]");
    print("Test Wolfe: p_nd = [ " + str(p_nd[0]) + " , " + str(p_nd[1]) + "]");
    ...
    second_tailor_approx += c_1 * (grad[0]*p_nd[0] + grad[1]*p_nd[1]) * a;
    ...
    if(phi_ak <= second_tailor_approx):    #Check for Wolfe criterion No.1
        ...
        new_grad_direct = new_grad[0]*p_nd[0] + new_grad[1]*p_nd[1]; #First term of wolfe 2
        print("Test Wolfe: new_grad = [ " + str(new_grad[0]) + " , " + str(new_grad[1]) + "]");
        approx_grad_direct = c_2 * (grad[0]*p_nd[0] + grad[1]*p_nd[1]); #Second term of wolfe 2
...
#Steepest descend direction
    neg_hess = fkt.neg_hessian(banana_a, banana_b, theta[0], theta[1]);
    grad = fkt.grad(banana_a, banana_b, theta[0], theta[1]);
    p_nd = [0,0];
    p_nd[0] = -1 * neg_hess[0][0]*grad[0] + neg_hess[0][1]*grad[1];
    p_nd[1] = -1 * neg_hess[1][0]*grad[0] + neg_hess[1][1]*grad[1];
    #def neg_hessian(a, b, x, y):
    #def grad(a, b, x, y)

    print("p_nd = [ " + str(p_nd[0]) + " , " + str(p_nd[1]) + " ]");
